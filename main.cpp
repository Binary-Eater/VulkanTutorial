...
private:
    ...
    void initWindow() {
        ...
        if (!glfwInit()) {
            ...
            case GLFW_PLATFORM_ERROR:
                throw std::runtime_error("GLFW failed to initialize due to a platform error!");
            default:
                throw std::runtime_error(
                    std::string("GLFW failed to initialize due to unknown error ") +
                    std::to_string(code));
            }
        }
    ...
    std::vector<const char*> getRequiredExtensions() {
        ...
        if (glfwExtensions == nullptr) {
            ...
            case GLFW_API_UNAVAILABLE:
                throw std::runtime_error("GLFW not able to find Vulkan API!");
            default:
                throw std::runtime_error(
                    std::string("GLFW get extensions failed with code ") +
                    std::to_string(code));
            }
        }
    ...
    void createLogicalDevice() {
        ...
        // TODO Will request features in the future
        VkPhysicalDeviceFeatures deviceFeatures{};
        ...
        /* Device specific validation layers not needed in latest Vulkan implementation */
        if (enableValidationLayers) {
            createInfo.enabledLayerCount = static_cast<uint32_t>(validationLayers.size());
            createInfo.ppEnabledLayerNames = validationLayers.data();
        } else {
    ...
    void createSurface() {
        ...
        /* Could use glfwCreateWindowSurface for platform agnostic WSI */
    }
